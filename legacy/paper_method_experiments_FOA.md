> **LEGACY / ARCHIVED — DO NOT USE AS ACTIVE RESEARCH GUIDANCE.**
> This is an obsolete paper draft for a *different* method (**AudioDepthFOA**: First-Order
> Ambisonic + spherical-harmonic guidance + covariance directional-energy maps). It does **not**
> describe the current project. The active method is **RayDPT** (binaural 5ch cues → ray-conditioned
> ERP depth); the current champion is eval-time L/R-flip TTA (commit 494b5e2). Preserved here only
> for historical record — see `EXPERIMENTS.md`, `hypotheses.tsv`, and `program.md` for the real state.
